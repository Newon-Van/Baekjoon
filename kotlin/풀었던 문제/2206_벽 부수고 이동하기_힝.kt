...
fun bfs(N: Int, M: Int): Int {
    ...
    while (queue.isNotEmpty()) {
        ...
        for (direction in directionArray) {
            ...
            if (next.row < 0 || next.row >= N || next.cul < 0 || next.cul >= M) // 벗어남?
                continue

            if (graph[next.row][next.cul].map == 1 && (visit[next.row][next.cul].chanceVisit || !now.breakChance)) // 벽인데 부숴서 온 적 있거나 부술 기회 있음?
                continue
            ...
            if (next.row == N - 1 && next.cul == M - 1)  // 정답임?
                return graph[now.row][now.cul].position + 1


            if (graph[next.row][next.cul].map == 1) {
                next.breakChance = false
                visit[next.row][next.cul].chanceVisit = true
            } else if (graph[next.row][next.cul].map == 0 && now.breakChance)
